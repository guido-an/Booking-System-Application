...
router.get("/complete-booking", (req, res) => {
  let userBookingWhen = {   // create an object with date, people and time properties 
    date: req.query.date,
...
router.post('/complete-booking', (req, res) => {
  ...
  transporter.sendMail({       // email to the ADMIN
  ...
  transporter.sendMail({      // email to the CUSTOMER  
    from: "Book Your Table",
    to: `${email}`,
